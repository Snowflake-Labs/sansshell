...
type fDBMoveClientFDBMoveDataWaitClientProxy struct {
	cc         *proxy.Conn
	directDone bool
	grpc.ClientStream
}

func (x *fDBMoveClientFDBMoveDataWaitClientProxy) Recv() ([]*FDBMoveDataWaitManyResponse, error) {
	var ret []*FDBMoveDataWaitManyResponse
	...
	if x.cc.Direct() {
		...
		m := &FDBMoveDataWaitResponse{}
		err := x.ClientStream.RecvMsg(m)
		ret = append(ret, &FDBMoveDataWaitManyResponse{
			Resp:   m,
			Error:  err,
...
//
// NOTE: The returned channel must be read until it closes in order to avoid leaking goroutines.
func (c *fDBMoveClientProxy) FDBMoveDataWaitOneMany(ctx context.Context, in *FDBMoveDataWaitRequest, opts ...grpc.CallOption) (FDBMove_FDBMoveDataWaitClientProxy, error) {
	stream, err := c.cc.NewStream(ctx, &FDBMove_ServiceDesc.Streams[0], "/Fdb.FDBMove/FDBMoveDataWait", opts...)
	if err != nil {
		return nil, err
	}
	x := &fDBMoveClientFDBMoveDataWaitClientProxy{c.cc.(*proxy.Conn), false, stream}
