...
// Embed the original client inside of this so we get the other generated methods automatically.
type stateClientProxy struct {
	*stateClient
}

// NewStateClientProxy creates a StateClientProxy for use in proxied connections.
// NOTE: This takes a proxy.Conn instead of a generic ClientConnInterface as the methods here are only valid in proxy.Conn contexts.
func NewStateClientProxy(cc *proxy.Conn) StateClientProxy {
	return &stateClientProxy{NewStateClient(cc).(*stateClient)}
}

// VersionManyResponse encapsulates a proxy data packet.
// It includes the target, index, response and possible error returned.
...
// VersionOneMany provides the same API as Version but sends the same request to N destinations at once.
// N can be a single destination.
//
// NOTE: The returned channel must be read until it closes in order to avoid leaking goroutines.
func (c *stateClientProxy) VersionOneMany(ctx context.Context, in *emptypb.Empty, opts ...grpc.CallOption) (<-chan *VersionManyResponse, error) {
	conn := c.cc.(*proxy.Conn)
	ret := make(chan *VersionManyResponse)
	// If this is a single case we can just use Invoke and marshal it onto the channel once and be done.
	if len(conn.Targets) == 1 {
		go func() {
			out := &VersionManyResponse{
				Target: conn.Targets[0],
				Index:  0,
				Resp:   &VersionResponse{},
			}
			err := conn.Invoke(ctx, "/Sansshell.State/Version", in, out.Resp, opts...)
			if err != nil {
				out.Error = err
			}
			// Send and close.
	...
	manyRet, err := conn.InvokeOneMany(ctx, "/Sansshell.State/Version", in, opts...)
	if err != nil {
		return nil, err
	}
	// A goroutine to retrive untyped responses and convert them to typed ones.
